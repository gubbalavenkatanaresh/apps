...
class AppStore extends Component {
  state = {apps: appsList, search: ''}

  selectedApps = tabId => {
    const filteredAppsList = appsList.filter(each => each.category === tabId)
    this.setState({apps: filteredAppsList})
  }
  ...
  render() {
    ...
    const filteredApps = apps.filter(eachApp =>
      ...
      <div className="container">
        ...
        <ul className="apps-list">
          {filteredApps.map(eachApp => (
            <AppItem appItem={eachApp} key={eachApp.appId} />
          ))}
        </ul>
      </div>
    )
